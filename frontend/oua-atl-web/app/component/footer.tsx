...
export default function FooterAlumniPage() {
  ...
  const [footerData] = useState<FooterSection>({
    "Quick Links": (
      <ul className="space-y-5">
        {quickLinks.map((link, index) => (
          <li key={index}>
            <a href={link.link} className="text-slate-500 text-nowrap hover:underline">
              {link.name}
            </a>
          </li>
        ))}
    ...
    "Funds Projects": <p>Coming Soon</p>,
    "Latest Tweets": <p>No tweets available</p>,
  });

  return (
    <div className="text-zinc-500 w-full justify-around bg-zinc-800 flex sm:flex-row p-32 pt-30 flex-col  items-start ">
      {/* Header */}
      <div className="mb-8 w-2/4">
        <h1 className="text-2xl font-bold mb-4">{headerData.title}</h1>
        <p className="text-[13px] font-sans">{headerData.description}</p>
        <button className="mt-2  w-full px-3 border rounded-md border-purple-500 p-2">
          Learn More
        </button>
      </div>

      {/* Grid Section */}
      <div className="ml-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {Object.entries(footerData).map(([sectionTitle, content], index) => (
          <div key={index} className="">
            <h2 className="text-[13px] font-[500] mb-2 border-b-2 border-b-purple-500 pb-2">
              {sectionTitle}
            </h2>
            <div className="text-sm font-sans space-y-5 ">{content}</div>
          </div>
        ))}
      </div>
    </div>
