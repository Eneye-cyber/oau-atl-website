  ...
  const [tagInput, setTagInput] = useState("");
    ...
    const form = useForm<FormValues>({
      ...
      defaultValues: {
        ...
        entranceFee: Number(event.entrance_fee ?? 0),
        tags: event.tags,
        ...
        ticketData: event.tickets.map(ticket => {
          const { price, title, starts_at, expires_at, rsvp_contacts, quantity_available } = ticket;
      
          const firstValidEmail = rsvp_contacts.find(contact =>
            /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(contact)
          ) || "";
    ...
    return (
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 mb-10">
          <div className="py-4">
            <h3 className="font-bold text-xl md:text-2xl">Update Event</h3>
          </div>
          <Card>
            <CardHeader>
