...
const SocialLinksSettings = ({ jsonData }: { jsonData: PageData }) => {
  ...
  const handleButtonInputChange =  (
      ...
      if (contentIndex !== null &&  section.content[contentIndex].list !== null) {
        // Update action field
        const listItem = section.content[contentIndex].list[listIndex] as ListItem
  ...
  return (
    <section className={`w-full ${isUploading ? "opacity-50 pointer-events-none" : ""}`}>
      <div className="border-y py-4 w-full my-6">
        <TopBar data={formData} userRole="guest" />
      </div>
                  ...
                  <div className="space-y-2">
                    ...
                    {content?.list?.length && (
                      content.list.map((listItem: any, listIndex) => (
                        <div className="grid sm:grid-cols-2 gap-3" key={listIndex}>
                          <div>
                            <Label
